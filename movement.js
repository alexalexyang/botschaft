...
const { getCurrentLocation, saveNextLocation, savePossibleLocations } = require('./DBHandlers/locationHandlers')
...
async function getPOIs(radius, lat, lng, poiKey, poiValue) {
    ...
    let POIs = await axios.get(query)
        .then(res => {
...
async function movementCycle(conn, botID) {
    setInterval(async () => {
        ...
        const [radius, lat, lng] = await getCurrentLocation(conn, botID).catch(err => console.log(err))
