async function getCurrentLocation(conn, botID) {
    ...
    let bot = await collection.find({ botID: botID }).sort({ _id: -1 }).limit(1).next().then(bot => { return bot }).catch(err => console.error(err))
    if (bot) {
        return [300, bot.lat, bot.lng]
    }

    collection = conn.db('botschaft').collection('bots')
    bot = await collection.findOne({ botID: botID }).then(bot => { return bot }).catch(err => console.error(err))
